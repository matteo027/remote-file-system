...
impl<B: RemoteBackend> RemoteFS<B> {
    ...
    fn entry_to_attr(&self, ino: u64, entry: &FileEntry, uid: u32, gid: u32) -> FileAttr {
        FileAttr {
            ino,
            size: entry.size,
            ...
            perm: entry.perms,
            nlink: entry.nlinks,
            uid, // modification needed due to macOS FUSE limitations
            gid, // modification needed due to macOS FUSE limitations
...
impl<B: RemoteBackend> Filesystem for RemoteFS<B> {
        ...
        match self.backend.list_dir(dir.to_str().unwrap()) {
            Ok(entries) => {
                if let Some(entry) = entries.iter().find(|e| e.name == name.to_string_lossy()) {
                    ...
                    let attr = self.entry_to_attr(ino, entry, req.uid(), req.gid());
    ...
    fn create(&mut self, req: &Request<'_>, parent: u64,name: &OsStr,_mode: u32,_umask: u32,flags: i32,reply: ReplyCreate,) {
        let dir = self
            .inode_to_path(parent)
            .unwrap_or_else(|| PathBuf::from("/"));
